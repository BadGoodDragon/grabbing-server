...
public interface MapItemRepository extends JpaRepository<MapItem, Long> {
    @Query(value = "insert into grabbing.map_item select nextval('grabbing.map_item_id_seq'), :object_id, :key, :value returning map_item.id", nativeQuery = true)
    Long create(@Param("object_id") Long objectId, @Param("key") String key, @Param("value") String value);
